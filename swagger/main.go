...
import (
	...
	_ "/gopractise-demo/swagger/docs"
	"log"
	"net/http"
)
...
func Create(c *gin.Context) {
	...
	for _, u := range users {
		if u.Name == user.Name {
			c.JSON(http.StatusBadRequest, gin.H{"message": fmt.Sprintf("user %s already exists", user.Name), "code": 10001})
			return
		}
...
// Get return the detail information for user.
